...
class PTB(object):
  ...
  def __init__(self, config, is_training, name=None):
    ...
    self.num_steps = 331
    self.input_data = tf.placeholder(tf.float32, shape=(None, 331))
...
def run_epoch(session, model, xdata, ydata, epoch_size, eval_op=None, verbose=False):
  ...
  return np.exp(costs / iters), costs / epoch_size
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    for i in range(config.max_max_epoch):
      ...
      train_perplexity, cost = run_epoch(session, m, train_x, train_y, train_epoch, eval_op=m.train_op,
                                         verbose=True)
      print("Epoch: %d Train Perplexity: %.3f Cost: %.3f" % (i + 1, train_perplexity, cost))
      valid_perplexity, cost = run_epoch(session, mvalid, valid_x, valid_y, valid_epoch)
      print("Epoch: %d Valid Perplexity: %.3f Cost: %.3f" % (i + 1, valid_perplexity, cost))

      test_perplexity, cost = run_epoch(session, mtest, test_x, test_y, test_epoch)
